(ns audio-port-fetcher.core
  (:require
   [audio-port-fetcher.init :as init]
...
(defn resp->filename
  ...
  (if-let [content-disposition (get headers "Content-Disposition")]
    ...
    (let [content-type (get headers "Content-Type")
      ...
      (log/warn "No content-disposition found in headers. Using sha-256-based name.")
      (str (sha-256 body) ext))))

(defn- download-file
  "Attemps to download the binary. If status code is 200, it is saved to disk."
  [url cookies]
  (log/info (str "Downloading audio file from '" url "'"))
  (let [resp (client/get url {:as :byte-array
    ...
    (if (= 200 (:status resp))
      (let [body     (:body resp)
        ...
        (log/info (str "Saving file to '" filename))
        (io/copy body (io/file filename)))
      (log/fatal (str "Unable to download file from URL '" url "'")))))
...
(defn- fetch-program-episode
  ...
  ;; don't thread these two into the block as execute-script seems to return `nil`
...
(defn- read-program-title
  ...
  (-> (elem/find-by-xpath* browser "//div[@id='content_title']")
      first
      elem/text
      (str/replace "Results from Series:" "")
      trim-text))
...
(defn- fetch-program-data-using-search
  ...
  [browser pub_title]
  (log/info (str "Searching for program using text '" pub_title "'"))
...
(defn- fetch-program-by-date
  ...
  (log/info (str "Fetching programs dated " fetch-date))
  (let [matched-eps (filter #(when (= (:date %) fetch-date) %) episodes)]
    (if (empty? matched-eps)
      (log/error "No episodes found matching date.")
...
(defn- fetch-program-files
  ...
  (doseq [program-code req-programs]
    (log/info "Processing program code" program-code)
      ...
      (if-let [episodes (-> (fetch-program-data browser prog)
        ...
        (cond
          ...
          (let [latest (first episodes)]
            (log/info (str "Fetching latest program dated " (:date latest)))
            (fetch-program-episode browser latest)))
        (log/fatal (str "No episodes found!")))
      (log/warn "Program code '" program-code "' not found in config.")))
  browser)
...
(defn -main
  [& args]
  (let [{:keys [action options programs exit-message ok?]} (init/validate-args args)]
    (if-not exit-message
      (case action
        "fetch" (fetch-programs programs options))
      (do ; error validating args
        (log/fatal exit-message)
        (System/exit (if ok? 0 1))))))

(comment
  ;; repl testing
  (def config (init/read-config default-config-file))
